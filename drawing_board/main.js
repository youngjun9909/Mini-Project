document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('pencil').addEventListener('click', () => {
    erasing = false  // 지우기 비활성
    toggleActiveButton('pencil') // 활성화 버튼 토글
  });

  document.getElementById('eraser').addEventListener('click', () => {
    erasing = true  // 지우기 비활성
    toggleActiveButton('eraser') // 활성화 버튼 토글
  });

  document.getElementById('clear').addEventListener('click', clearCanvas)
  document.getElementById('save').addEventListener('click', saveCanvas)
  document.getElementById('load-img').addEventListener('click', loadImg)
  ...
  function loadImg(e) {
    ...
    reader.onload = (event) => {
      const img = new Image();document.addEventListener("DOMContentLoaded", () => {
        const canvas = document.getElementById("drawing-board");
        const ctx = canvas.getContext("2d");
      ...
      img.onload = () => {
        ctx.drawImg(img, 0, 0, canvas.width, canvas.height);
      };
      img.src = event.target.result;
    };
