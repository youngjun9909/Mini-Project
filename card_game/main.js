...
document.addEventListener("DOMContentLoaded", () => {
  ...
  //! 색상 배열을 정의(6개)
  // 'black', 'red', 'yellow', 'pink', 'orange', 'blue' 등과 같은 영단어 이름 사용 가능
  const colors = [
    "#cc3131",
    "#e3ddbb",
    "#23b923",
  ...
  function initializeGame() {
    //? cardColors 배열을 섞기
    // : shuffle() 사용자 정의 함수 사용 - 배열 요소의 순서를 무작위로 섞음
    shuffle(cardColors);
  ...
  function flipCard() {
    ...
    //       >> flipCard 함수가 이벤트 리스너로써 연결된 HTML 요소(사용자가 클릭한 카드 요소를 가리킴)
    //       >> 함수가 이벤트 리스너로 사용될 때, this는 해당 이벤트가 발생한 요소를 자동으로 참조
...
function shuffle(array) {
  ...
  // : 배열의 길이는 1에서 시작, length - 1에서 시작

  // i는 11부터 0까지 반복 (요소가 12개)
  // >> 현재 요소(i)와 무작위로 선택된 요소(j)의 위치를 교환
  for (let i = array.length - 1; i > 0; i--) {
    // Math.random() * (i + 1): 0부터 i까지 무작위 인덱스를 생성
    // Math.floor(): 소수점 아래를 버리고 정수 인덱스를 확보
